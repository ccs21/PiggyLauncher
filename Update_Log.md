...
## 2026년 5월 6일
스팀 게임 이미지 경로가 계속 바뀌고 있어서 수집 방식을 변경 했습니다.

## 2026년 4월 23일
일부 스팀 게임의 썸네일, 헤더 이미지를 수집하지 못하는 문제를 수정 했습니다.
...
게임패드 자동 맵핑 DB 업데이트
- 03008fe54c050000c405000000006800 PS4 Controller
- 0300938d5e040000ff02000000007200 Controller (Xbox One For Windows)
- 0300cbe9632500007505000000000000 PS3/PC Gamepad
## 2026년 1월 6일
쿠팡 제휴 광고 로드 시 구성요소(웹뷰, VC++런타임) 누락으로 인한 프로그램 튕김 현상 수정
- 이제 누락된 구성요소가 있을 경우 설치를 안내 합니다.

게임패드 자동 맵핑 DB 업데이트
- 0300509d5e040000130b000013057200 Xbox Series X Controller
- 030057564c050000e60c000000006800 DualSense Wireless Controller

## 2026년 1월 3일
쿠팡 제휴 광고 삽입

## 2025년 12월 27일
게임패드 맵핑 시 방향키에 D-pad 뿐만 아니라, 아날로그 스틱도 할당이 가능하도록 수정

## 2025년 12월 21일
게임패드 자동 맵핑 DB 업데이트
- 03000166c82d00000b31000000007200 (8BitDo Ultimate 2 Wireless Controller for PC)

## 2025년 12월 20일
- 기존 게임이 실행중이거나 런쳐가 최소화 상태일 때 입력을 차단하던 코드를 런쳐가 활성 상태가 아니면 입력을 차단 하도록 수정
- 입력 차단 시 입력 감지 타이머 자체가 멈추도록 수정
- OS단에서 발생하는 키 이벤트도 차단할 수 있도록 코드를 수정
- 게임 실행 시 런쳐에서 인식하는 입력 초기화 후 차단
- 입력 차단이 순간적으로 풀렸다가 걸리는 경우에도 입력이 들어가지 않게 딜레이 추가
- 게임 실행중에는 런쳐에 키보드, 마우스, 터치, 제스쳐 등의 입력도 들어가지 않도록 방어 코드 강화
- 로그 분석의 정밀도를 위해 로그 정리 및 로그 메세지 수정
- 코드 안정화

## 2025년 12월 17일
게임 자동 등록 시 스팀 게임 스크린샷을 받아오지 못하는 문제 수정

에뮬레이터 롬 리스트 관련 최적화
- 리스트 리로드 방식 변경
- SA코어 변경 시 에뮬레이터 경로가 변경되지 않는 문제 수정

업데이트 시 계정이 로그아웃 되는 문제 수정

  
## 2025년 11월 24일
푸시알림 관련 수정 및 최적화
- 기존 : 게임 실행 중 푸시를 수신하면 런쳐설정의 알림목록에만 등록
- 수정 후 : 게임 실행 중 푸시를 수신하면 게임 종료->런쳐 복귀 후 알림 표시
- 기존 : 알림을 좌측하단 또는 좌측 상단에 3초간 표시
- 수정 후 : 알림을 좌즉하단 또는 좌측 상단에 10초간 표시
- 기존 : 알림의 상세 내용을 보기 위해 런쳐 설정의 알림목록으로 이동하여 해당 공지를 눌러야 함
- 수정 후 : 알림을 누르면 상세 내용 페이지로 자동 연결(상세내용이 없는 알림의 경우 알림창만 닫히고 반응 없음)

## 2025년 11월 23일
게임 실행시에도 런쳐에 패드 입력이 먹어서 여러 게임이 동시 실행 되는 문제 수정
- HHC의 게임패드 가상화기능과의 충돌이 원인이지만, 원천적으로 입력관련 문제를 차단하기 위해 업데이트 했습니다.

HHC 사용자 게임패드 충돌 이슈 수정
- 설정 부분에서 HHC 사용자 설정 항목을 추가

임시파일 삭제 권한 관련 오류창이 뜨는 부분 수정
- BAT/VBS 파일의 생성 위치를 변경
...
## 2025년 11월 15일
게임패드 자동 맵핑 DB 업데이트
- 030023da373500009910000000007200  Controller (GameSir-Dongle)
